/* ===================================================
   ...
   - 장소 CRUD + 검색
   - 장소 이미지, 카테고리, 리뷰
   - PLACES, PLACE_IMAGE, PLACE_CATEGORY, PLACE_REVIEW 테이블 사용
   =================================================== */
...
/* ── 1) 장소 목록 조회 (검색 + 필터) ── */
/* GET /api/places */
/* ?keyword=밀도 &region=마포구 &category=베이커리 */
router.get('/', async (req, res) => {
  try {
    const { keyword, region, category } = req.query;
    ...
    /* 키워드 검색 (장소 이름 또는 주소) */
    if (keyword) {
      query += ' AND (p.PLACE_NAME LIKE ? OR p.ADDRESS LIKE ?)';
