...
export default function HeroBanner() {
  ...
  useEffect(() => {
    ...
    const newMarkers = recommendedBakeries.map((bakery) => {
      ...
      /* 마커 생성: 동그란 빵 이모지 아이콘 마커 */
      /* DB에 빵집이 많으니까 심플한 원형 마커로 표시 */
      ...
      {selectedBakery && (
        <div className="hero-place-card">
          ...
          <div className="place-card-images">
            {/* 메인 이미지 (큰 이미지) - 나중에 실제 이미지로 교체 */}
            <div className="place-card-img main-img">
              <div className="img-placeholder">🍞 이미지 1</div>
            </div>
            {/* 서브 이미지 (작은 이미지) */}
            <div className="place-card-img sub-img">
              <div className="img-placeholder">🥐 이미지 2</div>
            </div>
          </div>

          {/* --- 빵집 이름과 위치 정보 --- */}
          <div className="place-card-info">
            {/* 빵집 이름 (선택된 빵집의 이름을 보여줌) */}
            <h2 className="place-name">{selectedBakery.name}</h2>
            {/* 빵집 주소 */}
